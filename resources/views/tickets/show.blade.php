{{-- ============================================ --}}
{{-- Tickets Show - Secure View dengan Komentar --}}
{{-- ============================================ --}}
...
@push('styles')
    <style>
        @media (min-width: 992px) {
            ...
            /* Scrollbar kustom untuk panel kanan */
...
@section('content')
    <div class="container-fluid px-md-5">
        <div class="row g-0 shadow-lg rounded-4 overflow-hidden border bg-white" style="min-height: 85vh; max-height: 85vh;">

            {{-- PANEL KIRI: METADATA & INFO --}}
            <div class="col-md-4 bg-light p-5 d-flex flex-column border-end overflow-auto scrollable-panel"
                ...
                <div class="mb-4">
                    ...
                    <div class="mb-3">
                        <span class="text-muted small fw-bold letter-spacing-1 d-block mb-1">TIKET
                            #{{ $ticket->id }}</span>
                        {{-- ✅ SAFE: Auto-escaped --}}
                ...
                <div class="flex-grow-1">
                    <div class="mb-4 p-4 rounded-4 bg-white shadow-sm border">
                        <label class="small fw-bold text-muted d-block mb-2 uppercase letter-spacing-1">STATUS &
                            PRIORITAS</label>
                        <div class="d-flex gap-2">
                            <span
                                class="badge {{ $ticket->status_badge ?? 'bg-primary' }} px-3 py-2 rounded-pill text-capitalize">
                                {{ str_replace('_', ' ', $ticket->status) }}
                            </span>
                            <span
                                class="badge {{ $ticket->priority_badge ?? 'bg-secondary' }} px-3 py-2 rounded-pill text-capitalize">
                    ...
                    {{-- Security Info Card (Diadaptasi ke UI Panel Kiri) --}}
                    <div class="p-4 rounded-4 bg-success bg-opacity-10 border border-success small">
                        ...
                        <ul class="list-unstyled mb-0 text-dark">
                            ...
                            <li class="mb-1"><i class="bi bi-check2 text-success me-1"></i> XSS Prevention (auto-escape)
                            </li>
                            <li class="mb-1"><i class="bi bi-check2 text-success me-1"></i> Input Validation &
                                Sanitization</li>
            ...
            <div class="col-md-8 p-5 bg-white d-flex flex-column scrollable-panel overflow-auto" style="max-height: 85vh;">

                {{-- DESKRIPSI MASALAH --}}
                <div class="mb-4">
                    <label class="form-label fw-bold small text-muted uppercase letter-spacing-1 mb-3">DESKRIPSI
                        MASALAH</label>
                    <div class="p-4 rounded-4 bg-light border-0 text-dark" style="line-height: 1.6;">
                        {{-- ✅ SAFE: e() untuk XSS prevention, nl2br untuk line breaks --}}
                ...
                <div class="mb-5 pb-4 border-bottom">
                    <label class="form-label fw-bold small text-muted uppercase letter-spacing-1 mb-3">KONTROL & AKSI
                        CEPAT</label>
                    <div class="d-flex flex-wrap gap-2">
                        @if ($ticket->user_id === auth()->id() || (auth()->user()->is_admin ?? false))
                            <a href="{{ route('tickets.edit', $ticket) }}"
                                class="btn btn-primary px-4 py-2 rounded-3 border-0 shadow-sm">
                        ...
                        <form action="{{ route('comments.store', $ticket) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-text mt-2 d-flex justify-content-between align-items-center">
                                    <span><i class="bi bi-shield-lock text-success me-1"></i> Input disanitasi dengan
                                        aman</span>
                                ...
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <div class="d-flex align-items-center">
                                        <div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center me-3"
                                            ...
                                            <span
                                                class="fw-bold">{{ strtoupper(substr($comment->user->name ?? 'U', 0, 1)) }}</span>
                                        </div>
                                        <div>
                                            <h6 class="mb-0 fw-bold">
                                                {{ $comment->user->name ?? 'Unknown User' }}
                                                @if ($comment->user_id === $ticket->user_id)
                                                    <span class="badge bg-info text-dark ms-2 rounded-pill"
                                    ...
                                    @if (auth()->id() === $comment->user_id || (auth()->user()->is_admin ?? false))
                                        <form action="{{ route('comments.destroy', $comment) }}" method="POST"
                                            ...
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="btn btn-sm text-danger btn-link p-0 text-decoration-none"
